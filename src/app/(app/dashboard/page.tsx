...
import { Book, Users, QrCode, Trophy } from 'lucide-react';
import { useLanguage } from '@/context/language-context';
import { Progress } from '@/components/ui/progress';

const completedSwaps = mockSwaps.filter((s) => s.status === 'completed');
const totalSwaps = completedSwaps.length;
const rewardGoal = 50;
    ...
    <div className="space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{totalSwaps * 2}</div>
            <p className="text-xs text-muted-foreground">
         ...
         <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Race to 50 Swaps</CardTitle>
            <Trophy className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{totalSwaps} / {rewardGoal}</div>
            <Progress value={(totalSwaps / rewardGoal) * 100} className="mt-2 h-2" />
             <p className="text-xs text-muted-foreground mt-1">
              Complete {rewardGoal} swaps for a special reward!
            </p>
          </CardContent>
        </Card>
      </div>
